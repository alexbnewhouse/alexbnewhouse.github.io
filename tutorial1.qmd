...
$\sum_{i=1}^n X_i$ symbolizes a _summation_. The sigma $\sum$ is the greek symbol for "add all of the elements together". The $i$ below the sigma indicates that we're going to start at the _first element_ of our data. The $n$ above the sigma indicates that we're going to use _all_ of the data up to the last value. $X_i$ refers to the $i$-th element of X, our vector holding all of the values that we want to average.
